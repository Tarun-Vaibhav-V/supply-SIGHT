...
def low_inventory(db: Session = Depends(get_db)):
...
def supplier_reliability(db: Session = Depends(get_db)):
...
def dropdown_products(db: Session = Depends(get_db)):
    result = db.execute(text("""
...
def update_shipment_status(shipment_id: int, body: StatusUpdate, db: Session = Depends(get_db)):
    """
...
def detect_risks(db: Session = Depends(get_db)):
    """
